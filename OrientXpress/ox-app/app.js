"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
//our root app component
var angular2_1 = require('angular2/angular2');
var customerService_1 = require('./customerService');
var customerBrowser_1 = require('./customerBrowser');
var App = (function () {
    function App(customerData) {
        this.customerData = customerData;
        this.customers = customerData.customers;
    }
    App.prototype.saidHello = function ($event) {
        alert("You said hello to " + $event);
    };
    App = __decorate([
        angular2_1.Component({
            selector: 'customer-app',
            providers: [customerService_1.CustomerData],
            template: "\n  <div class=\"row\">\n    <h2>Customer App\n  </div>\n  <customer-browser [customers]=\"customers | async\" class=\"row\"></customer-browser>\n      \n  ",
            directives: [customerBrowser_1.CustomerBrowser],
            encapsulation: angular2_1.ViewEncapsulation.None
        }), 
        __metadata('design:paramtypes', [customerService_1.CustomerData])
    ], App);
    return App;
}());
exports.App = App;
//# sourceMappingURL=app.js.map